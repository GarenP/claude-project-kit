...
**★ Honest boundary — read first (this loop cannot fully grade itself).** Every agent in this loop — the proposer, the runner, the scorer, and even the "adversarial" refuter — is the SAME model, fresh from the conclusions but NOT independent of the grader. It shares its own refuter's blind spots and priors. So this loop CANNOT reliably detect a failure mode it shares with its refuter, and it cannot, by itself, tell a genuinely-stressing battery from one that has quietly drifted into theater. Treat its self-assessment as provisional, not proof. The only real external referent is (a) the human's lived knowledge of their own product and (b) reality at the build gate — use both deliberately (see the sanity-sample + build-gate reconciliation notes).

Batteries are not static — they learn. The fuel is the **scoreboard**: in the experiment registry, every run records a performance row per lens — **raw found · confirmed-real (survived adversarial verification) · false-alarm rate · top severity · one-line verdict.** ⚠ **"Confirmed-real" measures refuter-survival (internal defensibility), NOT real-world hit rate** — a finding can be airtight-yet-trivial. So the scoreboard is **provisional**: it ranks *defensible* findings, which is not the same as *valuable* ones, and it must be reconciled against reality at the build gate (which findings actually corresponded to real problems). A lens that finds nothing on a clean spec is NOT bad (clean bill of health); "good" = *when real problems exist, it catches them with low noise AND high value* — judged over many runs, against the build-gate reality, not by refuter-survival alone.
