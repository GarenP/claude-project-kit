...
- **Semantic contradiction sweep (the graph's job — the deterministic checkers can't do it).** `check_doc_sync` catches STRUCTURAL drift (a doc citing a missing/superseded decision); it CANNOT catch SEMANTIC clashes (two decisions, or a decision and a doc, that contradict in *meaning* without citing each other). So after the checkers, read the relational decision-graph (the decision↔decision + decision↔doc edges) + the related ledger rows and flag any semantic contradiction as a `🟡 DISCUSS` recommendation for the Phase 4 gate. **The graph PROPOSES, the human DISPOSES — never auto-resolve.** This is a Claude-Code reading task over the deterministic nodes, NOT a heavyweight graphRAG query (see the project's contradiction-sweep note if it has one).
...
- Walk every PENDING/DISCUSS row the session settled and **promote it into its target(s)**, updating **ALL** of these as needed so they stay congruent:
  ...
  - **The deterministic decision layers** — refresh the index + the relational graph so they stay congruent with the just-promoted ledger: `python sync_decision_layers.py` (or, if the project lacks it, regenerate the decisions index + the graph viz from the ledger). LLM-free. **NEVER trigger a heavyweight graphRAG / cognee rebuild here** — it's brittle and is not the governance graph's source of truth (the deterministic viz is).
