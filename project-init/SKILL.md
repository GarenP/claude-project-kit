...
## Phase −1 — NEVER restart existing work (idempotency guard — check this FIRST)
Before doing ANYTHING, check whether this project is already initialized: does a `CLAUDE.md` / spec / `DECISIONS.md` / `REPO-MAP.md` already exist here? **If yes, STOP the full init — do NOT re-scaffold, do NOT overwrite, do NOT restart the interview from scratch.** Switch to *continue/enhance* mode: read the existing North Star + ledger + spec, tell the user "this project's already set up — I'll pick up where you left off, not start over," and only ADD what's genuinely missing (e.g. a file the scaffold expects but the project lacks) after confirming with them. A user who got a skill UPDATE must never be thrown back to square one. The user's project files are sacred; a skill update changes the *method*, never their accumulated work.
